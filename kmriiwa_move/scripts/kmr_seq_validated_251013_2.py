...
SAFE_POSE_DEG  = [-120, 60, 0, -60, 0, 60, 0]  # ← 安全位置（度）【新增常量】 
# =================================
...
def is_same_pose_deg(pose_a, pose_b, tol_deg):
    """判断两个关节角列表在容差内是否相同（长度按 JOINT_NAMES 对齐）"""
...
class KMRSeqValidated:
    ...
    # ---------------- BASE ----------------
    def stop_base(self): self.pub_cmd.publish(Twist())
    ...
    def move_arm_point_deg(self, degs, hold_cmd_s=1.0, wait_tol_deg=ANGLE_TOL_DEG, wait_timeout_s=12.0):
        ...
        if not ok:
            ...
            # 【新增】如果目标就是“安全位”，达到后再等待 1 秒
            if is_same_pose_deg(vals, SAFE_POSE_DEG, ANGLE_TOL_DEG):
    ...
    def move_arm_traj_deg(self, points_deg, times_s, wait_tol_deg=ANGLE_TOL_DEG, final_hold_check=True):
        ...
        for p_deg, t in zip(points_deg, times_s):
            vals = pad_to_len(p_deg, len(JOINT_NAMES), 0.0)
        ...
        r = rospy.Rate(PRINT_HZ)
        while not rospy.is_shutdown():
            state = self.fjt_client.get_state()  # 1: ACTIVE, 3: SUCCEEDED, etc.
            self.print_joints_10hz_once()
        ...
        if final_hold_check and len(points_deg) > 0:
            ...
            if not ok:
                ...
                # 【新增】若末点是“安全位”，达到后等待 1 秒
                if is_same_pose_deg(last, SAFE_POSE_DEG, ANGLE_TOL_DEG):
                    rospy.loginfo("Reached SAFE pose (trajectory). Extra wait 1.0s...")
                    rospy.sleep(1.0)
    ...
    def run(self):
        ...
        p1 = [90, 90, 0, -90, 0, 0, 0]
        p2 = [90, 90, 0,  90, 0, 0, 0]
        self.move_arm_traj_deg([p1, p2, SAFE_POSE_DEG], [2.0, 4.0, 6.0])
